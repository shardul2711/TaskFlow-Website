...
const EditTask = () => {
  ...
  // Load teammates and current task details
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Fetch task
        const taskRes = await taskService.getTaskById(id);
        ...
        const tagsStr = task.tags ? task.tags.join(', ') : '';
        setValue('tagsInput', tagsStr);

        if (task.attachment) {
          setAttachmentUrl(task.attachment);
            ...
            {!attachmentUrl ? (
              ...
              <div className="flex items-center justify-between p-3 rounded-btn border border-primary/20 bg-primary/5 dark:bg-primary/10">
                ...
                <button
                  ...
                  className="text-slate-455 hover:text-red-500 p-1 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
